...
class ReportOderList(Resource):
    def get(self):
        start_time = get_argument('start_time', default='2020-01-01')
        end_time = get_argument('end_time', default='2020-02-01')
        ...
        total_dic['total_ord_salenum_Global'] = sum([v for k, v in total_dic.items() if
        ...
        for source in source_code:
            if f'ord_salenum_{source}' in dic.keys():
                total_dic[f'total_ord_maoli_rate_{source}'] = (total_dic.get(f'total_ord_maoli_{source}', 0) / \
                                                               total_dic.get(f'total_ord_sale_amount_{source}', 0))
